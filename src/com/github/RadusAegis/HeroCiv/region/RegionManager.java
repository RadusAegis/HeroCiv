...
import com.github.RadusAegis.HeroCiv.ConfigManager;
import com.github.RadusAegis.HeroCiv.HeroCiv;
import com.github.RadusAegis.HeroCiv.PermSet;
import com.github.RadusAegis.HeroCiv.effect.Effect;
import com.github.RadusAegis.HeroCiv.events.RegionCreatedEvent;
import com.github.RadusAegis.HeroCiv.events.RegionDestroyedEvent;
import com.github.RadusAegis.HeroCiv.events.SuperRegionCreatedEvent;
import com.github.RadusAegis.HeroCiv.events.SuperRegionDestroyedEvent;
import org.bukkit.*;
...
public class RegionManager {
    ...
    public RegionManager(HeroCiv plugin, FileConfiguration config) {
        this.plugin = plugin;
        this.config = config;
        ...
        for (File currentRegionFile : regionFolder.listFiles()) {
            try {
                ...
                plugin.warning("[HeroCiv] failed to load " + currentRegionFile.getName());
                e.printStackTrace();
            }
        }
                ...
                if (locationString != null) {
                    ...
                    if (location != null && type != null) {
                        try {
                            ...
                            System.out.println("[HeroCiv] failed to load data from " + regionFile.getName());
                        }
                    }
                }
            } catch (Exception e) {
        ...
        for (File sRegionFile : sRegionFolder.listFiles()) {
            try {
                ...
                System.out.println("[HeroCiv] failed to load superregions from " + sRegionFile.getName());
                e.printStackTrace();
            }
        }
        ...
        try {
            ...
            log.warning("[HeroCiv] failed to load war.yml");
        }
    ...
    private ArrayList<ItemStack> processItemStackList(List<String> input, String filename) {
        ...
        for (String current : input) {
            ...
            if (Material.getMaterial(params[0]) != null) {
                ...
                plugin.warning("[HeroCiv] could not find item " + params[0] + " in " + filename);
            }
        }
        return returnList;
    }
    ...
    public void addRegion(Location loc, String type, ArrayList<String> owners) {
        ...
        try {
            ...
            dataConfig = new YamlConfiguration();
            System.out.println("[HeroCiv] saving new region to " + i + ".yml");
            ...
            System.out.println("[HeroCiv] unable to write new region to file " + i + ".yml");
            ioe.printStackTrace();
        }
    }
    ...
    public boolean addSuperRegion(String name, Location loc, String type, List<String> owners, Map<String, List<String>> members, int power, double balance) {
        ...
        try {
            dataFile.createNewFile();
            dataConfig = new YamlConfiguration();
            System.out.println("[HeroCiv] saving new superregion to " + name + ".yml");
            ...
            System.out.println("[HeroCiv] unable to write new superregion to file " + name + ".yml");
            ioe.printStackTrace();
            return false;
        }
    }
    ...
    public void destroyRegion(Location l) {
        ...
        if (!dataFile.exists()) {
            System.out.println("[HeroCiv] Unable to destroy non-existent region " + currentRegion.getID() + ".yml");
            return;
        }
        if (!dataFile.delete()) {
            System.out.println("[HeroCiv] Unable to destroy non-existent region " + currentRegion.getID() + ".yml");
            ...
            System.out.println("[HeroCiv] Successfully destroyed region " + currentRegion.getID() + ".yml");
        }
            ...
            for (Player p : plugin.getServer().getOnlinePlayers()) {
                try {
                    if (p.getLocation().distanceSquared(threadL) < 400) {
                        p.sendMessage(ChatColor.GRAY + "[HeroCiv] " + ChatColor.WHITE + regionTypeName + " was disabled!");
                        if (configManager.getExplode()) {
                            p.sendMessage(ChatColor.GRAY + "[HeroCiv] " + ChatColor.RED + "look out it's going to explode!");
                        }
                    }
    ...
    public void destroySuperRegion(String name, boolean sendMessage) {
        ...
        if (!dataFile.exists()) {
            System.out.println("[HeroCiv] Unable to destroy non-existent superregion " + name + ".yml");
            return;
        }
        if (!dataFile.delete()) {
            System.out.println("[HeroCiv] Unable to destroy non-existent superregion " + name + ".yml");
            ...
            System.out.println("[HeroCiv] Successfully destroyed superregion " + name + ".yml");
        }
        ...
        if (sendMessage) {
            ...
            new Runnable() {
                  ...
                  public void run()
                  {
                    plugin.getServer().broadcastMessage(ChatColor.GRAY + "[HeroCiv] " + ChatColor.RED + regionName + " was destroyed!");
                  }
            }.run();
        }
        removeWars(name);
    ...
    public synchronized void reduceRegion(SuperRegion sr) {
        ConfigManager cm = HeroCiv.getConfigManager();
        if (!cm.getUsePower()) {
            return;
        }
        ...
        if (currentPower == 25) {
            new Runnable() {
                  ...
                  public void run()
                  {
                    plugin.getServer().broadcastMessage(ChatColor.RED + "[HeroCiv] " + st + " reached 25 power! Destruction is near!");
                  }
            }.run();
            ...
            new Runnable() {
                  ...
                  public void run()
                  {
                    plugin.getServer().broadcastMessage(ChatColor.RED + "[HeroCiv] " + st + " reached 10 power! Destruction is at hand!");
                  }
            }.run();
            ...
            new Runnable() {
                  ...
                  public void run()
                  {
                    plugin.getServer().broadcastMessage(ChatColor.RED + "[HeroCiv] " + st + " reached 0 power!");
                  }
            }.run();
        }
    }
    ...
    public synchronized double addBalance(SuperRegion sr, double balance) {
        ...
        if (balance < 0) {
            if (newBalance < 0 && HeroCiv.econ != null) {
                String ownerName = sr.getOwners().get(0);
                double ownerBalance = HeroCiv.econ.bankBalance(ownerName).balance;
                if (newBalance + ownerBalance <= 0 && ownerBalance != 0) {
                    HeroCiv.econ.withdrawPlayer(ownerName, ownerBalance);
                    Player p = plugin.getServer().getPlayer(ownerName);
                    if (p != null && p.isOnline()) {
                        p.sendMessage(ChatColor.RED + "[HeroCiv] " + sr.getName() + " and you are out of money. Do something fast!");
                    }
                } else {
                    HeroCiv.econ.withdrawPlayer(ownerName, -newBalance);
                }
    ...
    public void setWar(SuperRegion sr1, SuperRegion sr2) {
        ...
        try {
            if (!warFile.exists()) {
                System.out.println("[HeroCiv] failed to load war.yml");
                return;
            }
        } catch (Exception e) {
            System.out.println("[HeroCiv] failed to load war.yml");
        ...
        if (hasWar(sr1, sr2)) {
            try {
                ...
                System.out.println("[HeroCiv] failed to remove war from war.yml");
                return;
            }
        } else {
            try {
                ...
                System.out.println("[HeroCiv] failed to save new war to war.yml");
                return;
            }
        }
    }
    ...
    public void removeWars(String name) {
        ...
        try {
            ...
            System.out.println("[HeroCiv] Failed to save war.yml");
        }
    }
    ...
    public ArrayList<String> getPermSets(Player p) {
        ...
        for (String s : possiblePermSets) {
            if (HeroCiv.perms.has(p, "HeroCiv.group." + s)) {
                sets.add(s);
            }
        }
        return sets;
